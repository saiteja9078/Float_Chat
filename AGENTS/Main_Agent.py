...
def decompose_query(state: State):
    ...
    state["dec_queries"] = [response.for_filter_agent, response.for_sql_agent]
    return state

def execute_queries(state: State):
...
state: State = {
    "user_query": "Get the adjusted salinity profiles for floats in Bay of Bengal",
    "dec_queries": [],
    "data": {}
}
